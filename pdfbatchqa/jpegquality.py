...
def computeJPEGQuality(image):
    """Returns JPEG quality of a JPEG image"""
    ...
    # Threshold that is set to 50 in original IM code. This will result in undefined quality
    # estimates for lower quality images. Lowering the threshold fixes this, but it's unclear
    # how reliable the resulting qualty estimates are for lower qualities (although they seem
    # accurate based on tests with ImageMagick JPEGs that were created at known quality levels,
    # and a comparison with Neil Krawetz's fotoforensics service).
    iThresh = 50
    ...
    if len(qdict) >= 1:
        ...
        for i in range(100):
            if ((qvalue < hashes[i]) and (qsum < sums[i])):
                continue
            if (((qvalue <= hashes[i]) and (qsum <= sums[i])) or (i >= iThresh)):
                return i+1
            break
    return -9999
